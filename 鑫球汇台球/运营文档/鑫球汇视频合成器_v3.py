...
# 搜索关键词（每段对应的精准配图搜索词，英文更好搜）
IMAGE_QUERIES = [
    "billiard club interior pool table",
    "billiard bar empty table",
    "basement billiard room",
    "customer playing pool table bar",
    "billiard table green felt closeup",
    "night billiard hall neon light",
    "cue chalk tip closeup",
    "beer drink billiard table",
    "billiard club lounge cozy",
]  # NOTE: IMAGE_QUERIES保留注解以兼容历史
...
def create_scene_image(text, output_path, index, total, scene_image_path=None):
    ...
    if scene_image_path is not None and os.path.exists(scene_image_path):
        try:
            ...
            # 如果尺寸匹配（我们生成的bg_*.png就是1080x1920），直接使用
...
async def main():
    ...
    for i, scene in enumerate(SCENES):
        ...
        try:
            ...
            print(f"    ✓ 图片已生成 ({bg_path.split('/')[-1] if bg_path else '无配图'})")
        except Exception as e:
            print(f"    ✗ 图片生成失败: {e}")
            return
