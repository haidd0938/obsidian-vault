---
name: xurl
description: UMBRELLA - X/Twitter via xurl CLI: post, search, DM, media, v2 API. Also covers the alternative x-cli tool for the same platform.
version: 1.1.1
...
## Alternative CLI: x-cli (was `xitter` skill)

The upstream `x-cli` (`https://github.com/Infatoshi/x-cli`) is an alternative Python-based tool that uses the same API credentials. The official `xurl` CLI is preferred, but `x-cli` is available as a uv-installed alternative.

### Install

```bash
uv tool install git+https://github.com/Infatoshi/x-cli.git
uv tool upgrade x-cli  # upgrade later
```

### Credentials

Same 5 X API secrets. x-cli reads them from `~/.config/x-cli/.env`:
...
- `X_API_KEY` + `X_API_SECRET` = app identity
- `X_BEARER_TOKEN` = app-level read
- `X_ACCESS_TOKEN` + `X_ACCESS_TOKEN_SECRET` = acting as your user

All five are needed. Missing any one produces confusing auth/permission errors.

### When to Use x-cli vs xurl

| Aspect | xurl (official) | x-cli (community) |
|--------|----------------|-------------------|
| Install | npm install -g xurl | uv tool install ... |
| Output | JSON to stdout | JSON or formatted |
| Credentials | config file or env vars | ~/.config/x-cli/.env |
| Maintenance | X dev platform | Community |

Choose `xurl` for raw API access and multi-app workflows. Choose `x-cli` for simpler quick-command interactions.
