...
description: 'UMBRELLA - Configure Hermes Agent for messaging platforms: Telegram bot integration, setup and troubleshooting, bot degradation diagnosis, and WeCom/Enterprise WeChat configuration.'
tags:
  - telegram
  - messaging
  - gateway
...
## 🤝 Communication Protocol (MANDATORY read)

**Before replying to any user message on Telegram, check the voice/text protocol:**

See `references/voice-text-communication-protocol.md`. Quick rule: voice input → voice reply via `text_to_speech()`. Text input → text reply. No exceptions unless TTS is broken, in which case explain + provide text.
...
For WeCom configuration and troubleshooting, see the reference file at `references/wecom-configuration-troubleshooting.md`. Key points:

- WeCom uses WebSocket (`wss://openws.work.weixin.qq.com`) instead of polling
- Common error `93019 invalid bot_id` — strip the `bot` prefix from the ID
- Configure via `WECOM_BOT_ID` / `WECOM_SECRET` in `.env` or `platforms.wecom` in `config.yaml`

## TTS Configuration Troubleshooting

If the bot starts speaking English instead of Chinese, or TTS fails entirely, see `references/tts-config-troubleshooting.md`. 

**Most common fix**: `tts.edge.voice` in config.yaml reverted to `en-US-AriaNeural` — change back to `zh-CN-XiaoxiaoNeural` and restart gateway.

## Telegram Bot Degradation Diagnosis

If the Telegram bot suddenly starts responding with very short messages (63-118 chars), switches to English, or loses memory, see the full 6-dimension root cause framework at `references/telegram-bot-dumb-down-diagnosis.md`.
...
5. Truncate SOUL.md if too long (>10K chars)