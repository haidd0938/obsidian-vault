...
description: UMBRELLA - Deploy n8n as standalone Docker service on macOS with one-click startup, webhook triggering, and workflow management.
...
## Workflow Integration (merged from `n8n-workflow-integration`)

This section covers triggering, managing, and building n8n workflows from Hermes.

### Trigger an Existing n8n Webhook
...
**Notification:** Webhook → Email/SMS/Telegram → Respond with confirmation
**Document processing:** Webhook → Python node → Store result → Return status
**Data collection:** Webhook → HTTP Request nodes → Transform → Store → Return summary

### Troubleshooting

| Issue | Fix |
|-------|-----|
| Webhook not found | Ensure workflow is active in n8n |
| Connection refused | `n8n-start` first |
| Auth errors | Set up n8n API key in settings |

### Best Practices

1. Design workflows to handle duplicate triggers (idempotent)
2. Include error-handling nodes in n8n workflows
3. Log all Hermes ↔ n8n interactions
4. Use n8n API keys for production workflows
5. Export n8n workflows regularly for backup
