...
description: UMBRELLA - Hermes & OpenClaw custom provider configuration (OpenAI-compatible API endpoints), health/balance monitoring, and auto-failover setup. Covers NVIDIA NIM, DeepSeek, local Ollama, fallback model routing, and provider switch fault diagnosis.
version: 1.1.0
...
## Critical: Provider Balance / Validity Verification

A valid API key does NOT mean the provider has usable free credits. Always verify before configuring:

### Trap: 402 Insufficient Funds

Some providers (Hyperbolic, Together AI, Fal.ai) accept `POST /v1/chat/completions` or model listing endpoints but return **402 Insufficient Funds** on actual generation calls (image, video, audio). The `/v1/models` list endpoint may work fine — this is misleading.

**Verification steps before assuming a provider is usable:**

```bash
# Step 1: Test chat endpoint (may pass even with zero balance)
curl -s -X POST https://api.hyperbolic.xyz/v1/chat/completions \
  -H "Authorization: Bearer $KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"meta-llama/Meta-Llama-3.1-8B-Instruct","messages":[{"role":"user","content":"hi"}],"max_tokens":5}'
# → 200 OK even with $0 balance! This is misleading.

# Step 2: Test the actual capability you need (image gen, etc.)
# This is the REAL test:
curl -s -X POST https://api.hyperbolic.xyz/v1/image/generation \
  -H "Authorization: Bearer $KEY" \
  -H "Content-Type: application/json" \
  -d '{"model_name":"FLUX.1-schnell","prompt":"test","width":512,"height":512,"steps":2,"cfg_scale":0}'
# → 402 {"detail": "Insufficient funds"} ← REAL balance status
```

### Trap: 429 Too Many Requests

After a 402, retrying aggressively can trigger a **429 Too Many Requests** rate limit that persists even after waiting. This happens because the provider's rate limiter tracks failed auth attempts.

**Resolution:** Wait 15+ seconds between attempts. If still 429 after 3 tries, the key is effectively dead — move on.

### Free Image Generation API Reality Check (May 2026)

| Provider | Registration | Free Tier Status | Notes |
|----------|-------------|-----------------|-------|
| **Together AI** | Requires credit card (❌) | $1/mo credit, 3400 FLUX images | **Requires billing info** |
| **Hyperbolic** | Email only (✅) | $0.03/image, $0 starter | Key works for listing but **402 on actual gen** |
| **SiliconFlow** | Chinese phone (✅) | 2000万免费tokens | Best option for Chinese users, not fully tested |
| **Replicate** | GitHub account (✅) | Limited free trials | Often requires payment after trial |
| **NVIDIA NIM** | Email only (✅) | 40 calls/min free | Text/vision only, no image gen |
| **Fal.ai** | Requires credit card (❌) | $1 free credit | Requires billing info |

**Lesson:** Always test the SPECIFIC API endpoint you need (image gen, video gen, etc.) — not just the chat endpoint — before configuring a provider as "working." A key that passes listing/chat tests can still be useless for your actual use case.

### Recommended Verification Flow

```python
def verify_image_gen_provider(provider_name, api_key, base_url):
    """Test if an image generation provider actually works."""
    # 1. Test chat (quick smoke)
    chat_ok = test_chat(api_key, base_url)
    if not chat_ok:
        return {"status": "fail", "reason": "chat endpoint failed"}
    
    # 2. Test image gen (the real test)
    img_ok, error = test_image_gen(api_key, base_url)
    if not img_ok:
        if "402" in error or "Insufficient funds" in error:
            return {"status": "nobalance", "reason": "key is valid but has $0 balance"}
...
This skill also covers **OpenClaw** (the standalone personal AI assistant). OpenClaw uses a **JSON configuration file** at `~/.openclaw/openclaw.json` rather than YAML like Hermes.

For comprehensive OpenClaw configuration including free API providers, gateway management, and troubleshooting, see `references/openclaw-configuration-full.md` (absorbed from `hermes/openclaw-configuration`).
...
| Provider registration | `custom_providers: [...]` (list) | `models.providers.{name}: {...}` (dict) |
| Default model | `model.default: <name>` | `agents.defaults.model.primary: "provider/model"` |
| Fallback model | Manual / prompt-level | Built-in `fallback` + `strategy: "auto-failover"` |
| Auth setup | API key in provider config | Separate `auth.profiles.{name}:default` entry |
| Service restart | `pkill -f hermes_cli.main gateway` | `openclaw gateway restart` |

### OpenClaw Configuration Pattern

#### Step 1: Add a Provider

In `~/.openclaw/openclaw.json`, add the provider under `models.providers`:
...
> **Note**: Model IDs use a forward-slash format for discovery (`meta/llama-3.3-70b-instruct`). Provider names in `providers` are short keys (`nvidia`, `ollama`). The combined identifier for model selection is `nvidia/meta/llama-3.3-70b-instruct` (provider/model-id).

#### Step 2: Configure Auth Profile

OpenClaw requires a separate auth profile entry:
...
#### Step 3: Set Primary Model with Auto-Failover

The most powerful feature — set a primary cloud model and a local fallback:
...
**⚠️ Known Pitfall**: OpenClaw v2026.3.28 validates the `model` object strictly. If `fallback` + `strategy` fields cause a validation error (`agents.defaults.model: Invalid input`), simplify to just `{"primary": "provider/model"}` — the fallback will still work if you define multiple models in `agents.defaults.models` but won't have explicit auto-failover routing.
...
# Check: "Runtime: running (pid XXXX, state active)" + "RPC probe: ok"
```
...
| Command | Purpose |
|---------|---------|
| `openclaw gateway restart` | Restart the LaunchAgent service |
| `openclaw gateway status` | Check service status + RPC health |
| `openclaw gateway call health` | Quick health check |
| `openclaw config get <path>` | Get a config value (dot-separated) |
| `openclaw config set <path> <value>` | Set a config value |
| `openclaw configure` | Interactive guided setup |

### Best Management Practices for OpenClaw

1. **Backup before editing**: `cp ~/.openclaw/openclaw.json ~/.openclaw/openclaw.json.bak`
2. **Validate JSON**: Use `python3 -m json.tool ~/.openclaw/openclaw.json` to check syntax
3. **Test with curl first**: Verify the API works standalone before configuring
4. **Restart gracefully**: `openclaw gateway restart` is safer than killing the process
5. **Voice-call plugin warning**: A disabled plugin with residual config causes harmless warnings in gateway output

### Common Pitfalls

| Pitfall | Symptom | Fix |
|---------|---------|-----|
| **Rich model object fails validation** | `agents.defaults.model: Invalid input` on gateway start | Simplify to `{"primary": "provider/model"}` only |
| **Missing auth profile** | Gateway ignores the provider | Add `auth.profiles.{name}:default` entry |
| **Wrong model ID format** | Model not found in agent turn | Use `provider/model-id` format (e.g., `nvidia/meta/llama-3.3-70b-instruct`) |
| **Port conflict after restart** | "EADDRINUSE" error | Use `openclaw gateway restart` (handles graceful replacement) |

## OpenClaw Gateway Reload Reference

For runtime reload issues after dist/web UI updates, see:
- [OpenClaw Gateway Reload Pitfalls](references/openclaw-gateway-reload.md) — dist file reload, kill vs stop/start, Node version mismatch
