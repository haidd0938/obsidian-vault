#!/bin/bash
# ── 智能 Provider 切换脚本 ──────────────────
# 用法: ./switch-provider.sh [nvidia|deepseek|gemini|status]
...
DEEPSEEK_API_KEY="be5c67e4da59b68e413e66e217654f1f915185bd01eb41d0c4ab78da1d399319"
DEEPSEEK_MODEL="deepseek-chat"
DEEPSEEK_MAX_CONTEXT="131072"

# Gemini 配置参数（用于视觉分析任务）
GEMINI_BASE_URL="https://generativelanguage.googleapis.com/v1beta/openai/"
GEMINI_API_KEY="AIzaSyAtko0pdpRNBKMp-gExfAHx9tevAe0Ya30"
GEMINI_MODEL="gemini-2.5-flash"
GEMINI_MAX_CONTEXT="1048576"
...
switch_to_gemini() {
    echo "🟣 切换到 Gemini（视觉分析）..."

    sed -i '' "s/^  provider:.*/  provider: gemini/" "$CONFIG"
    sed -i '' "s/^  default:.*/  default: $GEMINI_MODEL/" "$CONFIG"
    sed -i '' 's|^  base_url:.*|  base_url: '\"$GEMINI_BASE_URL\"'|' "$CONFIG"
    sed -i '' "s/^  max_context:.*/  max_context: $GEMINI_MAX_CONTEXT/" "$CONFIG"

    echo "✅ 已切换至 Gemini - $GEMINI_MODEL（1M上下文，支持视觉）"
}
